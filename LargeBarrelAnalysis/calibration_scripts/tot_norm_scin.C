 ...
 */
const double fNominalAnnihilationEdge = 1000000.0;
const double fNominalDeexcitationEdge = 1500000.0;
...
pair<double, double> getEdges(TH1D* totHist)
{
  ...
  for (int bin = 15; bin < totHist->GetNbinsX() - 5; ++bin)
  {
...
void tot_norm(string fileName, string calibJSONFileName = "calibration_constants.json", bool saveResult = false, string resultDir = "./",
              int minScinID = 1, int maxScinID = 192)
{
  ...
  if (fileScinTOT->IsOpen())
  {
    ...
    {
      TH1D* totHist = allScins->ProjectionY(Form("tot_scin_%d", scinID), scinID, scinID);
